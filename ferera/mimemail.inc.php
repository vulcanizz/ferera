...
/*
 +-------------------------------------------------------------------+
 |                     M I M E M A I L   (v2.3)                      |
 |                                                                   |
 | Copyright Gerd Tentler               www.gerd-tentler.de/tools    |
 | Created: Nov. 2, 2004                Last modified: Sep. 27, 2015 |
 +-------------------------------------------------------------------+
...
//--------------------------------------------------------------------------------------------------------
    ...
    function get_inl_data($html, $m, $css) {
      ...
      $host = 'http://' . preg_replace('%/$%', '', $HTTP_HOST);
    ...
    function check_body() {
      $this->body = preg_replace('/ (onMouseOver|onMouseOut|onClick)=".+?"/i', '', $this->body);
    ...
    function build_header() {
      $this->header = 'Return-Path: ' . $this->senderMail . $this->eol .
                      'From: ' . $this->senderName . ' <' . $this->senderMail . '>' . $this->eol .
                      'X-Sender: ' . $this->senderMail . $this->eol .
                      'X-Mailer: MIMEmail (PHP)' . $this->eol .
                      'MIME-Version: 1.0' . $this->eol;

      if($this->replyTo) $this->header .= 'Reply-To: ' . $this->replyTo . $this->eol;
      if($this->cc) $this->header .= 'Cc: ' . $this->cc . $this->eol;
      if($this->bcc) $this->header .= 'Bcc: ' . $this->bcc . $this->eol;
      if($this->saveDir) $this->header .= 'Subject: ' . $this->subject . $this->eol;
      ...
      $this->header .= 'X-Priority: ' . $priority . $this->eol .
                       'X-MSMail-Priority: ' . $ms_priority . $this->eol;

      if(count($this->attachments)) {
        $this->header .= 'Content-Type: multipart/mixed; boundary="' . $this->uid1 . '"' . $this->eol . $this->eol .
                         'This is a multi-part message in MIME format.' . $this->eol . $this->eol .
                         '--' . $this->uid1 . $this->eol;
      }

      if($this->type == 'HTML') {
        $this->header .= 'Content-Type: multipart/alternative; boundary="' . $this->uid3 . '"' . $this->eol . $this->eol .
                         '--' . $this->uid3 . $this->eol;
        $this->header .= 'Content-Type: text/plain; ' .
                         'charset="' . $this->charSet . '"' . $this->eol .
                         'Content-Transfer-Encoding: 8bit' . $this->eol . $this->eol .
                         preg_replace("/(\s*\r?\n\s*){2}/", '\\1\\1', @strip_tags($this->body)) . $this->eol . $this->eol .
                         '--' . $this->uid3 . $this->eol;

        if(count($this->inline)) {
          $this->header .= 'Content-Type: multipart/related; boundary="' . $this->uid2 . '"' . $this->eol . $this->eol .
                           '--' . $this->uid2 . $this->eol;
        }
      }
      ...
      if(count($this->inline)) {
        while(list($ftype, $arr) = each($this->inline)) {
          if(count($arr)) while(list($inlName, $data) = each($arr)) {
            ...
            $this->footer .= '--' . $this->uid2 . $this->eol .
                             'Content-Type: ' . $inlType . '; name="' . $inlName . '"' . $this->eol .
                             'Content-ID: <' . $inlName . '>' . $this->eol .
                             'Content-Disposition: inline; filename="' . $inlName . '"' . $this->eol .
                             'Content-Transfer-Encoding: base64' . $this->eol . $this->eol .
                             $data . $this->eol . $this->eol;
          }
        }
        $this->footer .= '--' . $this->uid2 . '--' . $this->eol . $this->eol;
      }

      if($this->type == 'HTML') $this->footer .= '--' . $this->uid3 . '--' . ($atts ? $this->eol . $this->eol : '');

      if(count($atts)) {
        while(list($filename, $file) = each($atts)) {
          $this->footer .= '--' . $this->uid1 . $this->eol .
                           'Content-Type: ' . $ftypes[$filename] . '; name="' . $filename . '"' . $this->eol .
                           'Content-Disposition: attachment; filename="' . $filename . '"' . $this->eol .
                           'Content-Transfer-Encoding: base64' . $this->eol . $this->eol .
    ...
    function send($recipients) {
      ...
      if($this->created) {
        $this->build_header();
        $message = $this->body . $this->eol . $this->eol . $this->footer;
        ...
        if($this->saveDir) {
          ...
          if($fp = @fopen($file, 'w')) {
            $mimemail = 'To: ' . $recipients . $this->eol . $this->header . $this->eol . $message;

            if(!fwrite($fp, $mimemail, strlen($mimemail))) {
              $this->error = 'Could not write to "' . $file . '"' . $this->eol;
            }
            fclose($fp);
            $ok = true;
          }
          else $this->error = 'Could not open "' . $file . '"' . $this->eol;
        }
        else {
          $php_ver = phpversion();
          ...
          if($php_ver < '4.0.5' || ($php_ver >= '4.2.3' && $safe_mode)) {
            $ok = @mail($recipients, $this->subject, $message, $this->header);
          }
          else {
            $options = ($this->useQueue ? '-odq ' : '') . '-i -f ' . $this->senderMail;
            $ok = @mail($recipients, $this->subject, $message, $this->header, $options);
          }

          if(!$ok) {
            $this->error = 'Error while sending e-mail';
            if(!strstr($recipients, ',')) $this->error .= ' to "' . $recipients . '"' . $this->eol;
            else $this->error .= $this->eol;
          }
        }
        $this->cnt++;
      ...
      else $this->error = 'MIME mail not created yet' . $this->eol;
